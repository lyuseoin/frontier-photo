...
export default async function NoticesPage({
  ...
  const [{ edit, open }, notices, admin] = await Promise.all([
    searchParams,
    getNotices(),
    isAdmin(),
  ]);
  ...
  return (
    <>
      ...
      {admin ? (
        <AdminEditBar editing={editing} basePath="/notices" what="공지" />
      ) : null}

      {editing ? (
        <AdminNotices notices={notices} openId={open ?? null} />
      ) : (
        <ReadView notices={notices} />
      )}
    </>
  );
}

function ReadView({ notices }: { notices: Notice[] }) {
  const pinned = notices.filter((n) => n.is_pinned);
  const rest = notices.filter((n) => !n.is_pinned);

  if (notices.length === 0) {
    return <EmptyState>아직 등록된 공지가 없어요.</EmptyState>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {rest.length > 0 ? (
        <section>
          {pinned.length > 0 ? (
            <h2 className="mb-2 text-sm font-bold text-slate-500">전체 공지</h2>
          ) : null}
