...
`/admin`에 한 번 로그인하면, 공지·일정·시간표 **화면 위쪽에 편집 버튼**이 생깁니다.
누르면 그 자리에서 바로 등록·수정·삭제할 수 있고(주소는 `?edit=1`), "편집 끝내기"를
누르면 원래 보던 화면으로 돌아옵니다. 공지 상세에서 "이 공지 수정"을 누르면 그 공지의
입력 폼이 열린 채로 뜹니다.

로그아웃 상태에서는 이 버튼들이 렌더되지 않고, 주소창에 `?edit=1`을 직접 쳐도
편집 화면은 나오지 않습니다. 저장·삭제 서버 액션도 각각 로그인 여부를 다시 확인합니다.
