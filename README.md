...
`public/culture-game/index.html` — 같은 내용을 게임으로 푼 버전입니다. 배포 후 `/culture-game/` 로 접속.

- **흐름**: 시작 화면 → 역할 고르기(5종) → 여권에서 나라 선택 → 나라별 퀴즈 4문항 → 입국 도장 → 최종 등급
- **문제**: 6개국 × 4문항 = 24문항. 객관식 · 인사말 · 참/거짓(오해 바로잡기) 세 유형
- 3문제 이상 맞히면 그 나라의 입국 도장 획득. 모든 문항에 해설이 붙습니다.
- 진행 상황(역할·나라별 최고 점수)은 `localStorage`에 저장되어 이어하기가 가능하며, 서버로 전송되지 않습니다.
